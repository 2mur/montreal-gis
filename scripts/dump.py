...
def create_plotly_popup(group, parameter, color, sensor_name):
    ...
    <body style="background-color:#111; margin:0;">
        ...
        # Logic: Build three plotly subplots within one container

        <div id="data-card" style="width:400px; height:800px; background:#111;">
            <div id="series-chart"></div> <div id="variance-chart"></div> <div id="ml-chart"></div> 
            <script>
            ...
            var layout = {{
                ...
                yaxis: {{ zeroline: true, zerolinewidth: 3, zerolinecolor: '#ff5e00' }},
                showlegend: false
            }};
...
def create_anomaly_map():
    ...
    # --- CSS for Grid Background and Locked UI ---
    map_css = """
    <style>
        .leaflet-container { background-color: #0a0a0a !important; }
        .grid-pattern { background-image: radial-gradient(#00ffcc 0.5px, transparent 0.5px); background-size: 20px 20px; }
    </style>
    """
    ...
    # Restrict panning to Montreal BBox + padding
    m.fit_bounds([[45.38, -74.00], [45.74, -73.44]]) 
    m.set_options(max_bounds_viscosity=1.0)
